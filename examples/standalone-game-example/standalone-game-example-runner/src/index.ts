...
class GameRunner {
    ...
    const container = ez("div", styles.container).append(
      ...
      this.addressBar = ez("div", styles.addressBar).append(
        ...
        ez("div", styles.urlInputGroup).append(
          ...
          this.loadUrlBtn = ez("button", styles.loadBtn)
            .setTextContent("Load Game"),
          this.stopBtn = ez("button", [styles.stopBtn, styles.hidden])
            .setTextContent("Stop"),
        ),
      ),
      
      // Game loader section (shown when no game is loaded)
      this.gameLoader = ez("div", [styles.gameLoader])
      .append(
        ez("div", styles.loaderSection).append(
          ez("h3").setTextContent("Or Drop Game File"),
          this.fileDropZone = ez("div", [styles.fileDropZone])
            .append(
        ...
        ez("div", styles.gameInfo).append(
          ...
          ez("div").setTextContent("• WebSocket URLs (ws:// or wss://): Connect to networked game servers"),
        ),
      ...
      this.clientContainer = ez("div", [styles.mmlClient, styles.hidden])
      .append(
        this.loading = ez("div", styles.loading)
          .append(ez("div", styles.loadingSpinner), ez("p").setTextContent("Loading game...")),
      )
    );
  ...
  private async loadGameFromContent(content: string, source: string) {
    try {
      ...
      console.log("🔗 Connecting client to document...");
      this.state.mmlClient.connectToDocument(this.state.gameDocument);
  ...
  private async loadGameFromUrl(url: string, updateUrl: boolean = true) {
    try {
      ...
      if (this.isWebsocketUrl(url)) {
        ...
        await this.loadGameFromContent(content, `URL: ${url}`);
      }
    } catch (error) {
  ...
  private async loadGameFromFile(file: File) {
    try {
      ...
      await this.loadGameFromContent(content, `file: ${file.name}`);
    } catch (error) {
      this.showStatus(`${error instanceof Error ? error.message : String(error)}`, "error");
    }
  }
