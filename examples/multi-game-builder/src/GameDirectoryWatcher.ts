...
import { GameDirectoryScanner, GameDirectory, GameConfig } from "./GameDirectoryScanner";
...
export class GameDirectoryWatcher {
  ...
  constructor(rootPath: string, orchestrator: GameBuildOrchestrator, mmlDocumentManager: GameMMLDocumentManager) {
  ...
  private async handleFileAdded(filePath: string): Promise<void> {
    ...
    if (pathParts.length >= 2) {
      ...
      if (fileName === 'index.html' && pathParts[1] === 'build' && this.currentGames.has(gameName)) {
