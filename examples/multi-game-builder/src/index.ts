...
export class MultiGameBuilder {
  ...
  constructor(options: MultiGameBuilderOptions) {
    ...
    if (options.watchMode) {
      this.watcher = new GameDirectoryWatcher(options.rootPath, this.orchestrator, this.mmlDocumentManager);
    }
  }
...
async function main() {
  ...
  if (options.serverOptions) {
    ...
    console.log(`🌐 CORS: ${options.serverOptions.corsEnabled ? `enabled (${JSON.stringify(options.serverOptions.corsOrigins)})` : 'disabled'}`);
  }
