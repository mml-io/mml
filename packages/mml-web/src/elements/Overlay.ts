...
import { ChildrenMirror } from "../utils/ChildrenMirror";
...
export class Overlay<G extends GraphicsAdapter = GraphicsAdapter> extends TransformableElement<G> {
  ...
  private childMirror: ChildrenMirror | null = null;
  ...
  private static attributeHandler = new AttributeHandler<Overlay<GraphicsAdapter>>({
    ...
    anchor: (instance, newValue) => {
      instance.props.anchor = parseEnumAttribute(newValue, OverlayAnchor, OverlayAnchor["top-left"]);
  ...
  static get observedAttributes(): Array<string> {
    return [...TransformableElement.observedAttributes, ...Overlay.attributeHandler.getAttributes()];
  }
  ...
  public connectedCallback(): void {
    ...
    if (!this.getScene().hasGraphicsAdapter() || this.overlayGraphics || this.overlayElement) {
      return;
    }

    // Make an element that will be used to render the overlay even if it is not attached to the DOM
    ...
    if (parentElement) {
      ...
      console.warn("An m-overlay element was found but getOverlayElement was not provided by the scene");
    }

    this.childMirror = new ChildrenMirror(this, this.overlayElement, {
      mirrorExisting: true,
      subtree: true,
      attributes: true,
      characterData: true,
  ...
  public disconnectedCallback(): void {
    ...
    this.childMirror?.dispose();
    this.childMirror = null;
