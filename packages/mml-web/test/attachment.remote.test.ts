...
describe.each(["virtual", "dom"] as const)(
  "m-element m-remote-document attachment [%s mode]",
  (mode) => {
    let ctx: ModeContext;
    beforeAll(async () => {
    ...
    test("test attachment via div", async () => {
      ...
      const group = ctx.createElement("m-group") as Group;
      ...
      const secondGroup = ctx.createElement("m-group") as Group;
