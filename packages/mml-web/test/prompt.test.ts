...
import { Prompt } from "../build/index";
import { testElementSchemaMatchesObservedAttributes } from "./schema-utils";
import { createModeContext, ModeContext } from "./test-mode-utils";

describe.each(["virtual", "dom"] as const)("m-prompt [%s mode]", (mode) => {
  let ctx: ModeContext;
  beforeAll(async () => {
  ...
  test("test attachment to scene", async () => {
    const { scene } = await ctx.createSceneAttachedElement<Prompt>("m-prompt");
  ...
  test("clicking child element should trigger prompt", async () => {
    const { element, scene } = await ctx.createSceneAttachedElement<Prompt>("m-prompt");
