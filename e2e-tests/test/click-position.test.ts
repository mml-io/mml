...
describe("m-plane position click", () => {
  ...
  test("can get right position on click with offset", async () => {
    ...
    const labelContentExample4 = await labelSelector?.evaluate((el) => el.getAttribute("content"));
    ...
    expect(xE4).toBeGreaterThan(2.50);
    expect(yE4).toBeLessThan(2.50);
